...
/// @title ClubVaultUsdg
/// @notice ClubVault on Robinhood Chain, settled in USDG. Invite-only coins. Same pool as a normal Aroma launch, a different
...
///
/// @dev What differs from ClubVault, and only this:
///
/// - The quote currency is USDG, an ERC-20 with 6 decimals, not a native
///   18-decimal gas token. The ticks and liquidities below are therefore
///   script/math/derive_pool_usdg.py's, not derive_pool.py's; the dollar
///   geometry they describe is the same.
/// - A launched token must sort above USDG so that USDG is currency0 and the
///   tick constants read the right way round. The auction's bot mines a
///   CREATE2 salt that guarantees it.
/// - Fees are held as the pool manager's ERC-6909 claims rather than taken
///   as tokens mid-swap. `take` inside a hook pays out of whatever USDG the
///   pool manager already holds, before the trader has paid in. ClubVault
///   gets away with that because the pool manager always holds native
///   currency, but a USDG balance is not guaranteed and a fresh chain has
///   none, so every buy would revert. Minting a claim moves no tokens. Claims
///   become USDG when someone withdraws, by which point the swaps that earned
///   them have settled.
/// - Launches come from the Club auction, not from whoever calls: the factory
///   accepts only ClubAuction and names the auction's winner as creator — the
///   root of the tree, with the creator's seats.
///
/// A separate contract from PoolVault rather than an extension of it, and
...
contract ClubVaultUsdg is IHooks, IUnlockCallback, ReentrancyGuard, Ownable2Step, EIP712 {
    using SafeERC20 for IERC20;
    using PoolIdLibrary for PoolKey;
    using CurrencyLibrary for Currency;

    // ---------------------------------------------------------------
    // Pool geometry — identical to PoolVaultUsdg, derived by
    // script/math/derive_pool_usdg.py for a 6-decimal quote currency.
    // ---------------------------------------------------------------
    ...
    uint256 public constant MIN_JOIN_USDC = 1e6;
    ...
    /// @dev currency0 of every club pool: USDG. Named USDC in the interface
    /// so that callers written against ClubVault read it unchanged.
    ...
    /// @dev What an unlock is for. The pool manager calls back into one
    /// function whatever the reason, so the payload says which.
    enum Unlock {
        Launch,
        Withdraw
    }

    constructor(address poolManager_, address usdg_, address owner_)
        Ownable(owner_)
        EIP712("Aroma Clubs", "1")
    {
        ...
        returns (PoolId poolId)
    {
        ...
        require(token > Currency.unwrap(USDC), "token must sort above USDG");
        require(
        ...
        poolManager.unlock(abi.encode(Unlock.Launch, abi.encode(token)));
        _launching = false;
    ...
    function unlockCallback(bytes calldata data) external override returns (bytes memory) {
        ...
        (Unlock action, bytes memory payload) = abi.decode(data, (Unlock, bytes));

        if (action == Unlock.Withdraw) {
            (address to, uint256 amount) = abi.decode(payload, (address, uint256));
            // Burning the claim credits this contract; taking pays it out.
            // The two cancel, so the unlock closes with nothing owed.
            poolManager.burn(address(this), USDC.toId(), amount);
            poolManager.take(USDC, to, amount);
            return "";
        }
    ...
    function claim() external nonReentrant returns (uint256 usdc) {
        ...
        if (usdc > 0) _payOut(msg.sender, usdc);
    ...
    function withdrawProtocolFees(address to) external onlyOwner nonReentrant returns (uint256 usdc) {
        ...
        usdc = protocolUsdc;
        protocolUsdc = 0;
        if (usdc > 0) _payOut(to, usdc);
        emit ProtocolFeesWithdrawn(to, usdc);
    }

    /// @dev Turns `amount` of this contract's fee claims into USDG sent to
    /// `to`. Every claim was minted by a swap that has since settled, so the
    /// pool manager holds the USDG behind it.
    function _payOut(address to, uint256 amount) private {
        poolManager.unlock(abi.encode(Unlock.Withdraw, abi.encode(to, amount)));
    }
